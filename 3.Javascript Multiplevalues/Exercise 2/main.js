...
shoppingList.splice(breadIndex, 1, `Banana`, `Eggs`);
let lastItem = shoppingList.pop();
shoppingList.sort();
let milkIndex = shoppingList.indexOf(`Milk`);
let bananaIndex = shoppingList.indexOf(`Banana`);
shoppingList.splice(bananaIndex + 1, 0, `Carrots`, `Lettuce`);
let newShoppingList = [`Juice`, `Pop`];
let combineShoppingList = shoppingList.concat(newShoppingList, newShoppingList);
let LastIndexOfPop = combineShoppingList.lastIndexOf('Pop');*/
